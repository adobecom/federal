...
`.trim();var Ze=(n,e)=>{switch(n.type){case"FeaturedCard":return Kn(n,e);case"LinksCard":return Vn(n);case"PromoCard":return Yn(n);default:}return""},Jn=({sections:n},e)=>`
...
`);document.adoptedStyleSheets=[...document.adoptedStyleSheets,ce];var Co=async n=>{let{gnavSource:e,mountpoint:a,unavEnabled:r,miloConfig:t,personalization:i}=n;if(!(e instanceof URL))throw E(`gnavSource is invalid: ${e}`),new c("gnavSource needs to be a URL object");try{pn(t)}catch(v){throw E(`Failed to initialize MiloConfig: ${v}`),new c(`Failed to initialize MiloConfig: ${v}`)}Tn(i),_n(n.localizeLink??(v=>v)),Cn(wn(n));let o=await pe(n);if(o instanceof c)throw E(o.message),o;let{mainNav:s,aside:l}=o;if(s instanceof c)throw E(s.message),s;let d=ie(s,r);if(d instanceof c)throw E(d.message),d;return await Je(d)(a),na(n)},Je=n=>async e=>{let a=Qe(n);document.querySelector("main")?.setAttribute("id","main-content"),e.innerHTML=a,e.classList.add("site-pivot"),e.querySelector("nav")?.showPopover();let r=[...e.querySelectorAll(".mega-menu ~ .feds-popup")];r.forEach(s=>{s.innerHTML=""});let t=n.components.filter(s=>s.type==="MegaMenu"),i=t.map(s=>s.content),o=await Promise.all(i.map(async(s,l)=>{try{let[d,v]=await s,h=t[l].title;return r[l].innerHTML=ee(d,r[l].id,h),v}catch(d){return[d]}}).flat());return e},Qe=({components:n,productCTA:e,unavEnabled:a})=>`
