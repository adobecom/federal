...
`;var Qe=({categories:e,links:n,placeholders:a})=>{let r=`
    <ul class="tabs" role="tablist">
      ${e.map(Vn).join("")}
      ${n.length?`<li class="product-links"><a class="feds-link" href="${_(n[n.length-1].href)}"${L(null,n[n.length-1].daaLl??n[n.length-1].text)}>${n[n.length-1].text}${D.chevronRight}</a></li>`:""}
    </ul>
  `.trim(),t=`
...
`);document.adoptedStyleSheets=[...document.adoptedStyleSheets,gn];var Lo=async e=>{let{gnavSource:n,mountpoint:a,unavEnabled:r,miloConfig:t,personalization:i}=e;if(!(n instanceof URL))throw S(`gnavSource is invalid: ${n}`),new c("gnavSource needs to be a URL object");try{pe(t)}catch(u){throw S(`Failed to initialize MiloConfig: ${u}`),new c(`Failed to initialize MiloConfig: ${u}`)}Te(i),Se(e.localizeLink??(u=>u)),Le(we(e));let o=await cn(e);if(o instanceof c)throw S(o.message),o;let{mainNav:s,aside:l}=o;if(s instanceof c)throw S(s.message),s;let d=sn(s,r,await j());if(d instanceof c)throw S(d.message),d;return await Qn(d)(a),na(e)},Qn=e=>async n=>{let a=ea(e);document.querySelector("main")?.setAttribute("id","main-content"),n.innerHTML=a,n.classList.add("site-pivot"),n.querySelector("nav")?.showPopover();let r=[...n.querySelectorAll(".mega-menu ~ .feds-popup")];r.forEach(s=>{s.innerHTML=""});let i=e.components.filter(s=>s.type==="MegaMenu").map(s=>s.content),o=await Promise.all(i.map(async(s,l)=>{try{let[d,u]=await s;return r[l].innerHTML=nn(d,r[l].id),u}catch(d){return[d]}}).flat());return n},ea=({components:e,productCTA:n,unavEnabled:a,placeholders:r})=>`
