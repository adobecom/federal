  ...
  `},ke=({iconHref:n,iconAlt:e,title:r,href:t,subtitle:a,badges:o=[],daaLl:i,daaLh:s})=>{let l=e!==null&&n!==null,d=k(s,i??r),u=l?`
    ...
    `:"",b=o.length===0?"":`
      <div class="feds-product-card__badges">
        ${o.map(({text:p,isFilled:m})=>`
          <span class="feds-product-card__badge${m?" feds-product-card__badge--filled":""}">
            ${p}
          </span>
        `).join("")}
      </div>
...
`.trim(),Te=n=>{let{href:e,label:r,isDarkBg:t,imageData:a}=n,o=a.lightThemeImageSrc?.trim()||a.darkThemeImageSrc?.trim()||"",i=a.darkThemeImageSrc?.trim()||a.lightThemeImageSrc?.trim()||"",s=a.mobileLightThemeImageSrc?.trim()||a.mobileDarkThemeImageSrc?.trim()||"",l=a.mobileDarkThemeImageSrc?.trim()||a.mobileLightThemeImageSrc?.trim()||"",d=a.lightThemeImageAlt||a.darkThemeImageAlt||"",u=a.darkThemeImageAlt||a.lightThemeImageAlt||"",b=a.mobileLightThemeImageAlt||a.mobileDarkThemeImageAlt||"",g=a.mobileDarkThemeImageAlt||a.mobileLightThemeImageAlt||"",p=!!o,m=!!i,v=!!s,y=!!l,h=p?`<img src="${H(o)}" alt="${d}" />`:"",x=m?`<img src="${H(i)}" alt="${u}" />`:"",w=v?`<img src="${H(s)}" alt="${b}" />`:"",U=y?`<img src="${H(l)}" alt="${g}" />`:"";return`<div class="feds-brand-container${t?" feds-dark-bg":""}">
...
`,le=document.createElement("style");le.textContent=Ke;document.head.appendChild(le);var yo=async n=>{let{gnavSource:e,mountpoint:r,unavEnabled:t,miloConfig:a,personalization:o}=n;if(!(e instanceof URL))throw L(`gnavSource is invalid: ${e}`),new c("gnavSource needs to be a URL object");try{ln(a)}catch(u){throw L(`Failed to initialize MiloConfig: ${u}`),new c(`Failed to initialize MiloConfig: ${u}`)}En(o),Tn(n.localizeLink??(u=>u)),wn(yn(n));let i=await se(n);if(i instanceof c)throw L(i.message),i;let{mainNav:s,aside:l}=i;if(s instanceof c)throw L(s.message),s;let d=ae(s,t);if(d instanceof c)throw L(d.message),d;return await Ve(d)(r),Je(n)},Ve=n=>async e=>{let r=Ye(n);e.innerHTML=r,e.classList.add("site-pivot");let t=[...e.querySelectorAll(".mega-menu ~ .feds-popup")];t.forEach(s=>{s.innerHTML=""});let a=n.components.filter(s=>s.type==="MegaMenu"),o=a.map(s=>s.content),i=await Promise.all(o.map(async(s,l)=>{let[d,u]=await s,b=a[l].title;return t[l].innerHTML=Qn(d,t[l].id,b),u}).flat());return e},Ye=({components:n,productCTA:e,unavEnabled:r})=>`
<nav>
  <ul>
