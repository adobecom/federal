...
`.trim(),Te=n=>{let{href:e,label:a,isDarkBg:r,imageData:t}=n,i=t.lightThemeImageSrc?.trim()||t.darkThemeImageSrc?.trim()||"",o=t.darkThemeImageSrc?.trim()||t.lightThemeImageSrc?.trim()||"",s=t.mobileLightThemeImageSrc?.trim()||t.mobileDarkThemeImageSrc?.trim()||"",l=t.mobileDarkThemeImageSrc?.trim()||t.mobileLightThemeImageSrc?.trim()||"",d=t.lightThemeImageAlt||t.darkThemeImageAlt||"",v=t.darkThemeImageAlt||t.lightThemeImageAlt||"",x=t.mobileLightThemeImageAlt||t.mobileDarkThemeImageAlt||"",c=t.mobileDarkThemeImageAlt||t.mobileLightThemeImageAlt||"",g=!!i,f=!!o,h=!!s,y=!!l,m=g?`<img src="${R(i)}" alt="${d}" />`:"",b=f?`<img src="${R(o)}" alt="${v}" />`:"",w=h?`<img src="${R(s)}" alt="${x}" />`:"",V=y?`<img src="${R(l)}" alt="${c}" />`:"";return`<div class="feds-brand-container${r?" feds-dark-bg":""}">
  ...
  </div>`.trim()},fn=n=>{let{data:e}=n;return Te(e)};var un=["appswitcher","help"],Q={cs:["cz"],da:["dk"],de:["at"],en:["africa","au","ca","ie","in","mt","ng","nz","sg","za"],es:["ar","cl","co","cr","ec","gt","la","mx","pe","pr"],et:["ee"],ja:["jp"],ko:["kr"],nb:["no"],pt:["br"],sl:["si"],sv:["se"],uk:["ua"],zh:["cn","tw"]},[W,In]=(()=>{let n,e,a,r=new Promise(t=>{e=t,a=setTimeout(()=>{n={},t(n)},5e3)});return[t=>{t&&!n&&(n=t,clearTimeout(a),e?.(n))},()=>r]})();function nn(n,e=!1){let s=(/uc_carts=/.test(document.cookie)?n:n?.filter(d=>d!=="cart"))??[],l=s.length??3;if(e){let d=s.filter(x=>un.includes(x)).length;return`calc(92px + ${d*32}px + ${d*.25}rem)`}return`calc(${l*32}px + ${(l-1)*.25}rem)`}var en=n=>{if(!n.prefix||n.prefix==="/")return"en_US";let e=n.prefix.replace("/","");if(e.includes("_")){let[r,t]=e.split("_").reverse();return`${r.toLowerCase()}_${t.toUpperCase()}`}if(e==="uk")return"en_GB";let a=Object.keys(Q).find(r=>Q[r].includes(e));return a?`${a.toLowerCase()}_${e.toUpperCase()}`:`${e.toLowerCase()}_${e.toUpperCase()}`},_e={Mac:"macOS",Win:"windows",Linux:"linux",CrOS:"chromeOS",Android:"android",iPad:"iPadOS",iPhone:"iOS"},an=()=>{let n=navigator.userAgent;for(let[e,a]of Object.entries(_e))if(n.includes(e))return a;return"linux"},rn=async()=>{let n=window;return n.alloy?await n.alloy("getIdentity").then(e=>e?.identity?.ECID).catch(()=>{}):void 0};var Rn=()=>{try{return C().signInContext||{}}catch{return{}}},Me=()=>{let n=C();return j("signin-cta-style")==="primary"||n?.unav?.profile?.signInCtaStyle==="primary"?"primary":"secondary"},Pe=()=>{let e=C()?.unav?.profile?.messageEventListener;return e||(a=>{let{name:r,payload:t,executeDefaultAction:i}=a.detail;if(!(!r||r!=="System"||!t||typeof i!="function"))switch(t.subType){case"AppInitiated":window.adobeProfile?.getUserProfile().then(o=>{W(o)}).catch(()=>{W({})});break;case"SignOut":i();break;case"ProfileSwitch":Promise.resolve(i()).then(o=>{o&&window.location.reload()});break;default:break}})};function Ae(){let{unav:n}=C();return n?.unavHelpChildren||[{type:"Support"},{type:"Community"}]}var Z=()=>{let n=C();return{profile:{name:"profile",attributes:{accountMenuContext:{sharedContextConfig:{enableLocalSection:!0,enableProfileSwitcher:!0,miniAppContext:{logger:{trace:()=>{},debug:()=>{},info:()=>{},warn:()=>{},error:()=>{}}},complexConfig:n?.unav?.profile?.complexConfig||null,...n?.unav?.profile?.config},messageEventListener:Pe()},signInCtaStyle:Me(),isSignUpRequired:!1,callbacks:{onSignIn:()=>{window.adobeIMS?.signIn(Rn())},onSignUp:()=>{window.adobeIMS?.signIn(Rn())}}}},appswitcher:{name:"app-switcher"},notifications:{name:"notifications",attributes:{notificationsConfig:{applicationContext:{appID:n?.unav?.uncAppId||"adobecom",...n?.unav?.uncConfig}}}},help:{name:"help",attributes:{children:Ae()}},jarvis:{name:"jarvis",attributes:{appid:n?.jarvis?.id,callbacks:n?.jarvis?.callbacks}},cart:{name:"cart"}}};var Hn=(n,e)=>{n[0]&&"attributes"in n[0]&&n[0].attributes&&typeof n[0].attributes=="object"&&"isSignUpRequired"in n[0].attributes&&(n[0].attributes.isSignUpRequired=e)},tn=async(n,e)=>{try{let a=n.querySelector(".feds-utilities");if(!(a instanceof HTMLElement))return new u('missing ".feds-utilities" container');let r=new Set,t=document.head.querySelector('meta[name="universal-nav"]'),i=t instanceof HTMLMetaElement?t.content??"":"";t instanceof HTMLMetaElement||r.add(new u('metadata "universal-nav" is missing'));let o=i.trim();t instanceof HTMLMetaElement&&o.length===0&&r.add(new u('metadata "universal-nav" has no value'));let s=!window.adobeIMS?.isSignedInUser(),l=o.split(",").map(m=>m.trim()).filter(m=>Object.keys(Z()).includes(m)||m==="signup");if(s){let m=nn(l,s);a.style.setProperty("min-width",m)}let d;try{d=C()}catch{throw new Error("MiloConfig not available for UNAV initialization")}let v=en(d.locale),x=d.env.name==="prod"?"prod":"stage",c=await rn(),g=new URLSearchParams(window.location.search).get("unavVersion");/^\d+(\.\d+)?$/.test(g??"")||(g="1.5"),await Promise.all([ln(`https://${x}.adobeccstatic.com/unav/${g}/UniversalNav.js`),sn(`https://${x}.adobeccstatic.com/unav/${g}/UniversalNav.css`,!0)]);let f=()=>{let m=Z(),b=[m.profile];return Hn(b,!1),l?.forEach(w=>{if(w==="profile")return;if(w==="signup"){Hn(b,!0);return}let V=m[w];V&&b.push(V)}),b},h=()=>({target:a,env:x,locale:v,countryCode:zn(d?.locale)?.country||"US",imsClientId:window?.adobeid?.client_id,theme:"light",analyticsContext:{consumer:{name:"adobecom",version:"1.0",platform:"Web",device:an(),os_version:navigator.platform},event:{visitor_guid:c}},children:f(),isSectionDividerRequired:!!d?.unav?.showSectionDivider,showTrayExperience:!A.matches});await window?.UniversalNav?.(h()),s||a?.style.removeProperty("min-width");let y=m=>{window?.UniversalNav?.reload(h())};return A.addEventListener("change",()=>{y()}),{reloadUnav:y,errors:r}}catch(a){let r=a instanceof Error?a.message:"failed to load universal nav";return new u(r)}};function D(n,e){return[...n.querySelectorAll(e)]}function K(n,e,a){D(n,e).forEach(r=>a?r.removeAttribute("tabindex"):r.setAttribute("tabindex","-1"))}var B={ArrowLeft:-1,ArrowRight:1,ArrowUp:-1,ArrowDown:1},Dn=new Set(["ArrowLeft","ArrowRight"]),ze=new Set(["ArrowUp","ArrowDown"]),$e='.tabs [role="tab"][aria-selected="true"]';function mn(n,e,a){return(n+e+a)%a}function Ie(n,e,a,r){let t=B[a];if(Dn.has(a)){let g=e+t;return g>=0&&g<n.length?g:null}let i=getComputedStyle(r).gridTemplateColumns.split(" ").length,o=[...r.children],s=g=>{let f=n[g].parentElement;return f?o.indexOf(f):-1},l=s(e)%i,d=Math.floor(s(e)/i)+(a==="ArrowDown"?1:-1),v=Math.floor((o.length-1)/i);if(d<0||d>v)return null;let x=null,c=1/0;for(let g=0;g<n.length;g++){let f=Math.abs(s(g)%i-l);Math.floor(s(g)/i)===d&&f<c&&(c=f,x=g)}return x}function vn(n){K(n,'.tab-content [role="tabpanel"] a',!1);let e=[];D(n,".feds-popup[popover]").forEach(c=>{let g=()=>{c.matches(":popover-open")||K(c,'[role="tabpanel"] a',!1)};c.addEventListener("toggle",g),e.push(()=>c.removeEventListener("toggle",g));let f=!1,h=m=>{m.key==="Tab"&&!m.shiftKey&&(f=!0)};c.addEventListener("keydown",h),e.push(()=>c.removeEventListener("keydown",h));let y=m=>{if(f&&!c.contains(m.relatedTarget)){if(c.hidePopover?.(),!A.matches){let b=c.closest(".feds-gnav-items");b?.classList.remove("subscreen-opening"),b?.classList.add("subscreen-closing")}f=!1}};c.addEventListener("focusout",y),e.push(()=>c.removeEventListener("focusout",y))});let a=(c,g)=>{c.focus(),g.preventDefault()},r=()=>n.querySelector(".feds-popup:popover-open"),t=c=>c.querySelector($e),i=c=>c.querySelector('[role="tabpanel"]:not([hidden])');function o(c){let g=r(),f=n.querySelector("#feds-menu-wrapper");if(!f)return!1;let h=f.querySelector(".feds-gnav-items"),y=g?g.querySelector(".feds-popup-back-button"):null,m=h?.classList.contains("subscreen-opening")===!0;if(g!==null&&m&&y!==null)return y.click(),c.preventDefault(),!0;let b=g??(f?.matches(":popover-open")?f:null);if(!b)return!1;b.hidePopover?.();let w=g?`[popovertarget="${b.id}"]`:".feds-nav-toggle";return n.querySelector(w)?.focus(),c.preventDefault(),!0}function s(c,g,f){if(!Dn.has(g))return!1;let h=D(n,".feds-gnav-items > li > .feds-link"),y=h.indexOf(c);return y<0?!1:(a(h[mn(y,B[g],h.length)],f),!0)}function l(c,g,f,h){let y=D(g,'.tabs :is([role="tab"], .product-links a)'),m=y.indexOf(c);if(m<0)return!1;if(B[f]){let b=y[mn(m,B[f],y.length)];return b.matches('[role="tab"]')&&b.click(),a(b,h),!0}if(f==="Tab"&&!h.shiftKey&&c.matches('[aria-selected="true"]')){let b=i(g);if(!b)return!1;K(b,"a",!0);let w=b.querySelector("a");return w&&a(w,h),!0}return!1}function d(c,g,f,h){let y=i(g);if(!y)return!1;let m=D(y,"a"),b=m.indexOf(c);if(b<0)return!1;if(B[f]){let w=Ie(m,b,f,y);return w!==null?(a(m[w],h),!0):f==="ArrowUp"?(K(y,"a",!1),a(t(g)??m[0],h),!0):!1}if(f==="Tab"&&!h.shiftKey){if(b+1<m.length)a(m[b+1],h);else return!1;return!0}if(f==="Tab"&&h.shiftKey){if(b>0)a(m[b-1],h);else{K(y,"a",!1);let w=t(g)??D(g,'.tabs :is([role="tab"], .product-links a)')[0];w&&a(w,h)}return!0}return!1}function v(c,g,f,h){if(!ze.has(f))return!1;let y=D(g,".feds-gnav-cards a"),m=y.indexOf(c);return m<0?!1:(a(y[mn(m,B[f],y.length)],h),!0)}function x(c){let g=document.activeElement??c.target;if(c.key==="Escape"){o(c);return}let f=r();f&&(f.matches(":has(.product-list)")&&(l(g,f,c.key,c)||d(g,f,c.key,c))||f.matches(":has(.feds-gnav-cards)")&&v(g,f,c.key,c))||s(g,c.key,c)}return n.addEventListener("keydown",x),e.push(()=>n.removeEventListener("keydown",x)),()=>e.forEach(c=>c())}var Re="feds-milo",L=(n,e="default",a="e")=>{let{locale:r}=C(),t=j("gnav-source")??`${r.contentRoot??""}/gnav`;window.lana||console.warn("lana logging unavailable in the gnav"),window?.lana?.log(`${n} | gnav-source: ${t} | href: ${window.location.href}`,{clientId:Re,sampleRate:1,tags:e,errorType:a})};var p=class n extends Error{constructor(e){super(e),Object.setPrototypeOf(this,n.prototype)}},u=class n extends Error{constructor(e,a="Minor"){super(e),Object.setPrototypeOf(this,n.prototype),a==="Critical"&&L(e)}};var Un=n=>e=>{if(e===null)throw new Error("");let a=e.querySelector(He(n));if(!a)throw new Error("");let r=a.textContent??"",[t="",i=""]=r.split("|").map(x=>x.trim());if(t==="")throw new Error("");let o=a.getAttribute("href")??"";if(o==="")throw new Error("");let s=a.getAttribute("daa-ll"),l=a.getAttribute("aria-label")?.trim()??"",d=l!==""?l:i!==""?i:void 0,v=[];return[{type:n.type,text:t,href:o,ariaLabel:d,daaLl:typeof s=="string"&&s.trim()!==""?s:t},v]},hn=Un({type:"PrimaryCTA"}),z=Un({type:"SecondaryCTA"}),Nn=n=>O(hn).or(z).eval(n),He=({type:n})=>{switch(n){case"PrimaryCTA":return"strong > a";case"SecondaryCTA":return"em > a";default:throw new Error("")}};var bn=({text:n,href:e,daaLl:a,ariaLabel:r,ariaAttrs:t})=>`
<a href="${E(e)}"
  class="feds-primary-cta"${F(t,r)}
  ${k(null,a??n)}
>
  ${n}
</a>
...
<a href="${E(e)}" 
  class="feds-secondary-cta"${F(t,r)}
  ${k(null,a??n)}
>
  ${n}
</a>
`,jn=n=>n.type==="PrimaryCTA"?bn(n):$(n);var G=({text:n,href:e,daaLl:a,ariaLabel:r,ariaAttrs:t,svgIcon:i=""})=>`<a class="feds-link" href="${E(e)}"${F(t,r)}${k(null,a??n)}>${n}${i}</a>`;var qn=n=>{let[e,a]=De(n);return[{type:"LinksCard",card:e},a]},De=n=>{let e=n.querySelector("h2, h3, h4")||null;if(!e)throw new p("Expected links card title");let a=n.querySelector("em > a"),r=[...n.querySelectorAll("a")].filter(l=>l!==a);if(r.length===0)throw new p("Expected at least one link");let[t,i]=_(r,T),[o,s]=(()=>{try{return z(n)}catch{return[null,[]]}})();return o&&(o.daaLl=`${e.textContent??""} - ${o?.daaLl}`),[{type:"LinksCardItem",title:e.textContent??"",links:t,footerCTA:o},[...i,...s]]};var Bn=n=>{let e=[...n.querySelectorAll("li > div")],a=[...n.querySelectorAll("li > a")],[r,t]=_(e,Ue),[i,o]=_(a,T);return[{type:"ProductList",categories:r,links:i},[...t,...o]]},Ue=n=>{let e=n.firstElementChild;if(e?.nodeName!=="H2")throw new p("Expected H2");let a=e.textContent??"",r=e.textContent??"",t=Ln(e),[i,o]=_(t,pn);return[{type:"ProductCategory",name:a,daaLl:r,links:i},o]};var Gn=n=>{let[e,a]=Ne(n);return[{type:"FeaturedCard",card:e},a]},Ne=n=>{let e=n.querySelector("h5")||null;if(!e)throw new u("Eye brow element not found");let a=n.querySelector("h4")||null,r=a?.nextElementSibling||null;if(!a)throw new p("Expected title");if(!r)throw new p("Expected subtitle");let t=r.nextElementSibling?.firstElementChild||null;if(!t)throw new p("Expected card link after subtitle");let[i,o]=T(t),[s,l]=z(n);return[{type:"Card",title:a.textContent??"",subtitle:r.textContent??"",bodyLink:i,footerCTA:s,eyeBrow:e.textContent??""},[...l,...o]]};var P={MissingBackgroundImageSection:"Promo card is missing background image section",MissingBackgroundImage:"Promo card is missing background image",MissingBackgroundImageAlt:"Promo card background image is missing alt text",MissingBackgroundImageSrc:"Promo card background image is missing src",MissingContentSection:"Promo card is missing card details section",MissingIcon:"Promo card is missing icon",MissingIconSrc:"Promo card icon is missing src",MissingIconAlt:"Promo card icon is missing alt text",MissingTitleElement:"Promo card is missing title element",MissingTitleText:"Promo card is missing title text",MissingSecondaryCtaAnchor:"Promo card is missing secondary CTA anchor",MissingPriceLink:"Promo card is missing price link"},On=n=>{let[e,a]=n.querySelectorAll(":scope > div"),r=new Set;if(e===void 0)throw new p(P.MissingBackgroundImageSection);let t=e.querySelector(":scope picture:not(:scope p picture) img")??null;t===null&&r.add(new u(P.MissingBackgroundImage));let i=t?.getAttribute("alt")??"";i===""&&r.add(new u(P.MissingBackgroundImageAlt));let o=t?.getAttribute("src")??"";if(o===""&&r.add(new u(P.MissingBackgroundImageSrc)),a===void 0)throw new p(P.MissingContentSection);let s=a.querySelector('a[href$=".svg"]')??null;s===null&&r.add(new u(P.MissingIcon));let[l,d]=(s?.textContent?.split("|")??["",""]).map(w=>w.trim());l===""&&r.add(new u(P.MissingIconSrc)),d===""&&r.add(new u(P.MissingIconAlt));let v=a.querySelector('p > a:not([href$=".svg"])')??null,x=v?.textContent?.trim()??"",c=v?.getAttribute("href")??"",g=c?Cn(c):!1;v===null&&r.add(new u(P.MissingPriceLink));let f=a.querySelector("p > strong")??null;if(f===null)throw new p(P.MissingTitleElement);let h=f?.textContent??"";h===""&&r.add(new u(P.MissingTitleText)),a.querySelector("em > a")===null&&r.add(new u(P.MissingSecondaryCtaAnchor));let[m,b]=(()=>{try{return z(n)}catch{return[null,[]]}})();return b.forEach(w=>r.add(w)),m&&(m.daaLl=`${h} - ${m?.daaLl}`),[{type:"PromoCard",card:{bgImageAlt:i,bgImageSrc:o,iconAlt:d,iconSrc:l,title:h,cta:m,priceText:x,priceHref:c,isPriceMerchLink:g}},[...r]]};var Xn=n=>{let e=new Set;if(n===null)throw new p(Fn.elementNull);let a=n.querySelector("h2")?.textContent??"";a===""&&e.add(new u(Fn.noTitle));let r=(async()=>{try{let t=n.querySelector("a"),i=new URL(t?.href??""),o=await N(i);if(o instanceof p)throw new Error(o.message);let s=await Mn(o);if(s instanceof p)throw new Error(s.message);return _n(i.href,s),n.classList.contains("product-list")?Bn(s):je(s)}catch(t){throw new p(t?.message)}})();if(r instanceof p)throw r;return[{type:"MegaMenu",title:a,content:r},[...e]]},Fn={elementNull:"Element is null",noTitle:"Large Menu has no Title"},je=n=>{let e=[...n.children];if(e.length===0)throw new p("No mega menu items found (did you forget to add them correctly?)");let[a,r]=_(e,t=>qe(t));if(a.length===0)throw new p("Failed to parse gnav cards sections");return[{type:"GnavCards",sections:a},r]},qe=n=>{let e=[...n.querySelectorAll(".featured-card, .links-card, .promo-card")];if(e.length===0)throw new p("Column contains no cards (did you forget to label them correctly?)");let[a,r]=_(e,t=>Be(t));if(a.length===0)throw new p("Failed to parse cards in column");return[{type:"GnavColumn",cards:a},r]},Be=n=>{if(n.classList.contains("featured-card"))return Gn(n);if(n.classList.contains("links-card"))return qn(n);if(n.classList.contains("promo-card"))return On(n);throw new p("Unsupported gnav cards section")};var Wn=({card:n},e)=>Ge(n,e),Ge=({title:n,subtitle:e,eyeBrow:a,footerCTA:r,bodyLink:t},i)=>{let o=`featured-eyebrow-${S(a)}`;return`
  <div class="featured-card" tabindex="0" aria-label="${a} ${i}" ${k(a,"")} role="group">
    <div>
      ...
      <span>${G({...t,ariaAttrs:{"aria-describedby":o},svgIcon:I.chevronRight})}</span>
    </div>
...
`.trim();var Kn=({card:n})=>Fe(n),Fe=({bgImageAlt:n,bgImageSrc:e,iconAlt:a,iconSrc:r,title:t,cta:i,priceText:o,priceHref:s,isPriceMerchLink:l})=>`
...
`.trim();var Xe=(n,e)=>{switch(n.type){case"FeaturedCard":return Wn(n,e);case"LinksCard":return Zn(n);case"PromoCard":return Kn(n);default:}return""},Vn=({sections:n},e)=>`
  <div class="feds-gnav-cards">
    ${n.map(a=>`<li>${a.cards.map(r=>Xe(r,e)).join("")}</li>`).join("")}
  </div>
`;var Yn=async({categories:n,links:e})=>{let a=await U(),r=`
...
.featured-card .feature-card-hint {
  display: none;
}
...
`);document.adoptedStyleSheets=[...document.adoptedStyleSheets,de];var Co=async n=>{let{gnavSource:e,mountpoint:a,unavEnabled:r,miloConfig:t,personalization:i}=n;if(!(e instanceof URL))throw L(`gnavSource is invalid: ${e}`),new p("gnavSource needs to be a URL object");try{dn(t)}catch(v){throw L(`Failed to initialize MiloConfig: ${v}`),new p(`Failed to initialize MiloConfig: ${v}`)}En(i),Sn(n.localizeLink??(v=>v)),kn(xn(n));let o=await le(n);if(o instanceof p)throw L(o.message),o;let{mainNav:s,aside:l}=o;if(s instanceof p)throw L(s.message),s;let d=te(s,r);if(d instanceof p)throw L(d.message),d;return await Ve(d)(a),Je(n)},Ve=n=>async e=>{let a=await Ye(n);document.querySelector("main")?.setAttribute("id","main-content"),e.innerHTML=a,e.classList.add("site-pivot"),e.querySelector("nav")?.showPopover();let r=[...e.querySelectorAll(".mega-menu ~ .feds-popup")];r.forEach(s=>{s.innerHTML=""});let t=n.components.filter(s=>s.type==="MegaMenu"),i=t.map(s=>s.content),o=await Promise.all(i.map(async(s,l)=>{let[d,v]=await s,x=t[l].title;return r[l].innerHTML=await Qn(d,r[l].id,x),v}).flat());return e},Ye=async({components:n,productCTA:e,unavEnabled:a})=>`
<nav popover="manual" data-lenis-prevent>
  <a href="#main-content" class="feds-skip-link">${(await U()).get("skip-to-main")??"Skip to main content"}</a>
  <ul>
    ${(()=>{let t=n.find(d=>d.type==="Brand")??null,i=n.filter(d=>d.type!=="Brand"),o=`
      ...
      `.trim(),s=t?yn(t):"",l=Pn(i,yn);return`
...
`,Je=async n=>{let e=new Set,a=await tn(n.mountpoint);a instanceof u?(e.add(a),L(a.message)):a.errors.forEach(o=>e.add(o)),oe(n.mountpoint),vn(n.mountpoint),Qe(n.mountpoint),na(n.mountpoint),ea(n.mountpoint),ra(n.mountpoint),oa(n.mountpoint),ta(n.mountpoint,n.mepMartech??"");let r=()=>{document.querySelector("nav[popover]")?.removeAttribute("popover"),X(n.mountpoint)};document.querySelector(".dialog-modal")&&r(),document.addEventListener("click",o=>{o.target instanceof Element&&o.target.closest('a[href*="#openPrivacy"]')&&r()}),window.addEventListener("milo:modal:loaded",r),window.addEventListener("milo:modal:closed",()=>{let o=document.querySelector("nav");o?.setAttribute("popover","manual"),o?.showPopover?.()}),(await se(n.mountpoint)).forEach(o=>{e.add(o),L(o.message)});let i=a instanceof u?()=>{}:a.reloadUnav;return{closeEverything:aa,reloadUnav:i,errors:e,setGnavTopPosition:o=>{},getGnavTopPosition:()=>0}},Qe=n=>{let e=n.querySelector("#feds-menu-wrapper"),a=n.querySelector(".feds-nav-toggle");e?.addEventListener("toggle",()=>{let t=e.matches(":popover-open");a?.setAttribute("aria-expanded",String(t)),a?.setAttribute("daa-ll",t?"hamburgermenu|close":"hamburgermenu|open"),t&&e.classList.add("feds-menu-active")}),e?.addEventListener("transitionend",()=>{e.matches(":popover-open")||e.classList.remove("feds-menu-active")}),n.querySelectorAll(".feds-popup[popover]").forEach(t=>{t.addEventListener("toggle",()=>{let i=n.querySelector(`[popovertarget="${t.id}"]`),o=t.matches(":popover-open");i?.setAttribute("aria-expanded",String(o)),i?.setAttribute("daa-ll",o?"header|Close":"header|Open")})})},na=n=>{A.addEventListener("change",()=>{X(n)})},ea=n=>{[...n.querySelector(".feds-utilities #universal-nav")?.children??[]].forEach(e=>{e.addEventListener("click",()=>X(n)),e.addEventListener("keydown",a=>{a.key==="Enter"&&X(n)})})};var aa=()=>{},ra=n=>{let e=n.closest("header");if(!e)return;let a=n.querySelector("#feds-menu-wrapper"),r=()=>a?.matches(":popover-open")??!1,t=()=>window.scrollY>100,i=()=>{if(r()){e.classList.remove("feds-header-scrolled");return}if(t()){e.classList.add("feds-header-scrolled");return}e.classList.remove("feds-header-scrolled")};i(),window.addEventListener("scroll",i,{passive:!0}),a?.addEventListener("toggle",i)},ta=(n,e)=>{let a=n.closest("header");a!==null&&a.setAttribute("daa-lh",`gnav|${$n()}${e}`)},oa=n=>{n.querySelector(".feds-skip-link")?.addEventListener("click",a=>{let r=document.querySelector("#main-content");r instanceof HTMLElement&&(a.preventDefault(),r.hasAttribute("tabindex")||r.setAttribute("tabindex","-1"),setTimeout(()=>{r.focus(),r.scrollIntoView({behavior:"smooth",block:"start"})},100))})};export{Co as main,Je as postRenderingTasks,Ve as renderGnav,Ye as renderGnavString};
