...
`.trim();var Zn=({card:n})=>Ge(n),Ge=({title:n,links:e,footerCTA:a})=>`
...
`.trim();var Vn=({card:n})=>Oe(n),Oe=({bgImageAlt:n,bgImageSrc:e,iconAlt:a,iconSrc:r,title:t,cta:o,priceText:i,priceHref:s,isPriceMerchLink:l})=>`
...
`.trim();var Fe=n=>{switch(n.type){case"FeaturedCard":return Wn(n);case"LinksCard":return Zn(n);case"PromoCard":return Vn(n);default:}return""},Kn=({sections:n})=>`
  <ul class="feds-gnav-cards">
    ${n.map(e=>`<li>${e.cards.map(a=>Fe(a)).join("")}</li>`).join("")}
  </ul>
...
`,Qn=(n,e,a)=>{let r=`
  ...
  `.trim(),i="";switch(n.type){case"ProductList":i=Yn(n);break;case"GnavCards":i=Kn(n);break;default:}return`${o}${i}`};var ne={elementNull:"Error when parsing text. Element is null",textContentNull:"Error when parsing text. Element has no textContent"},ee=n=>{if(n===null)return[{type:"Text",content:""},[new f(ne.elementNull,"Minor")]];let e=n.textContent;return e===null?[{type:"Text",content:""},[new f(ne.textContentNull,"Minor")]]:[{type:"Text",content:e},[]]};var ae=({content:n})=>n;var re=n=>{if(n===null)throw new p(We.elementNull);let e=n.querySelector(".gnav-brand");if(e!==null)return cn(e);let a=n.querySelector(".large-menu");return a!==null?Xn(a):n.querySelector("strong")!==null?vn(n):n.querySelector("em")!==null?A(n):n.querySelector("a")===null?ee(n):T(n.querySelector("a"))},bn=(n,e)=>{switch(n.type){case"Text":return ae(n);case"Link":return q(n);case"SecondaryCTA":return z(n);case"PrimaryCTA":return hn(n);case"Brand":return gn(n);case"MegaMenu":return Jn(n,e);default:return console.error(`Failed to recognize component: ${n}`),""}},We={elementNull:"Element is null"};var te=(n,e)=>{let[a,r]=S([...document.querySelectorAll(".breadcrumbs ul > li > a")??[]],T),[t,o]=S([...n.children],re),i=n.querySelector(".product-entry-cta"),[s,l]=(()=>{try{return Nn(i)}catch{return[null,[]]}})(),d=!1,m=[r,o,l].flat();return{breadcrumbs:a,components:t,productCTA:s,localnav:d,errors:m,unavEnabled:e}};var oe=n=>{let e=[...n.querySelectorAll('.tabs button[role="tab"]')],a=[...n.querySelectorAll(".tab-content ul")],r=e.map((t,o)=>()=>{let i=a[o].closest(":popover-open"),s=i?.clientHeight??0;if(e.forEach(d=>{d.setAttribute("aria-selected","false")}),a.forEach(d=>{d.setAttribute("hidden","true")}),a[o]?.removeAttribute("hidden"),t.setAttribute("aria-selected","true"),!P.matches)return;let l=i?.clientHeight??0;i?.animate([{height:`${s}px`},{height:`${l}px`}],{duration:350,iterations:1,easing:"cubic-bezier(0.4, 0, 0.2, 1)"})});return e.forEach((t,o)=>{t.addEventListener("click",r[o])}),Ze(n),()=>{e.forEach((t,o)=>{t.removeEventListener("click",r[o])})}},Ze=n=>{let e=[...n.querySelectorAll(".feds-gnav-items > li > button")],a=[...document.adoptedStyleSheets.flatMap(t=>[...t.cssRules])].find(t=>t?.selectorText==="header.global-navigation nav::after");e.forEach(t=>{if(!a)return;let o=t.nextElementSibling;o&&o.addEventListener("toggle",i=>{if(i.newState==="open"){let s=o.offsetHeight??0;a.style.height=`${s+72}px`}else a.style.height="100%"})}),window.addEventListener("resize",()=>{if(!a)return;let t=n.querySelector(".feds-popup:popover-open");if(!t)return;let o=t.offsetHeight??0;a.style.height=`${o+72}px`});let r=n.querySelector(".feds-gnav-items");e.forEach(t=>{t.addEventListener("click",()=>{if(P.matches||!r)return;let o=t.nextElementSibling;o&&(r.classList.remove("subscreen-closing"),r.classList.add("subscreen-opening"),o.querySelector(".feds-popup-back-button")?.addEventListener("click",()=>{r.classList.remove("subscreen-opening"),r.classList.add("subscreen-closing"),setTimeout(()=>o.hidePopover(),240)}))})}),P.addEventListener("change",()=>{r?.classList.remove("subscreen-opening"),r?.classList.remove("subscreen-closing")}),n.querySelector(".feds-nav-toggle")?.addEventListener("click",()=>{r?.classList.remove("subscreen-opening"),r?.classList.remove("subscreen-closing")})};var ie=async n=>{let e=new Set,a=n.querySelectorAll("a.merch");if(a.length===0)return e;try{let r=C(),{base:t}=r;if(!t)return e.add(new f("base not found in config, cannot initialize merch links")),e;let o=await import(`${t}/blocks/merch/merch.js`),{default:i}=o;if(!i)return e.add(new f("decorateMerchLink not found in merch module")),e;a.forEach(s=>{i(s)})}catch(r){e.add(new f(`Error initializing merch links: ${r}`))}return e};var se=async({gnavSource:n,asideSource:e})=>{let a=await U(n);if(a instanceof p)return a;let r=await U(e);return{mainNav:a,aside:r}};var le=new CSSStyleSheet;le.replaceSync(String.raw`/**
...
.feds-brand {
  ...
  padding: 0 var(--s2a-spacing-lg) 0 var(--s2a-spacing-md);
  column-gap: 10px;
}
...
`);document.adoptedStyleSheets=[...document.adoptedStyleSheets,le];var bo=async n=>{let{gnavSource:e,mountpoint:a,unavEnabled:r,miloConfig:t,personalization:o}=n;if(!(e instanceof URL))throw L(`gnavSource is invalid: ${e}`),new p("gnavSource needs to be a URL object");try{ln(t)}catch(m){throw L(`Failed to initialize MiloConfig: ${m}`),new p(`Failed to initialize MiloConfig: ${m}`)}En(o),Tn(n.localizeLink??(m=>m)),wn(yn(n));let i=await se(n);if(i instanceof p)throw L(i.message),i;let{mainNav:s,aside:l}=i;if(s instanceof p)throw L(s.message),s;let d=te(s,r);if(d instanceof p)throw L(d.message),d;return await Ve(d)(a),Ye(n)},Ve=n=>async e=>{let a=Ke(n);e.innerHTML=a,e.classList.add("site-pivot"),e.querySelector("nav")?.showPopover();let r=[...e.querySelectorAll(".mega-menu ~ .feds-popup")];r.forEach(s=>{s.innerHTML=""});let t=n.components.filter(s=>s.type==="MegaMenu"),o=t.map(s=>s.content),i=await Promise.all(o.map(async(s,l)=>{let[d,m]=await s,b=t[l].title;return r[l].innerHTML=Qn(d,r[l].id,b),m}).flat());return e},Ke=({components:n,productCTA:e,unavEnabled:a})=>`
<nav popover="manual">
  <ul>
...
`,Ye=async n=>{let e=new Set,a=await an(n.mountpoint);a instanceof f?(e.add(a),L(a.message)):a.errors.forEach(o=>e.add(o)),oe(n.mountpoint),mn(n.mountpoint),Je(n.mountpoint),Qe(n.mountpoint),na(n.mountpoint),aa(n.mountpoint),ra(n.mountpoint,n.mepMartech??""),(await ie(n.mountpoint)).forEach(o=>{e.add(o),L(o.message)});let t=a instanceof f?()=>{}:a.reloadUnav;return{closeEverything:ea,reloadUnav:t,errors:e,setGnavTopPosition:o=>{},getGnavTopPosition:()=>0}},Je=n=>{let e=n.querySelector("#feds-menu-wrapper"),a=n.querySelector(".feds-nav-toggle");e?.addEventListener("toggle",()=>{let t=e.matches(":popover-open");a?.setAttribute("aria-expanded",String(t)),a?.setAttribute("daa-ll",t?"hamburgermenu|close":"hamburgermenu|open"),e.setAttribute("aria-hidden",String(!t)),t&&e.classList.add("feds-menu-active")}),e?.addEventListener("transitionend",()=>{e.matches(":popover-open")||e.classList.remove("feds-menu-active")}),n.querySelectorAll(".feds-popup[popover]").forEach(t=>{t.addEventListener("toggle",()=>{let o=n.querySelector(`[popovertarget="${t.id}"]`),i=t.matches(":popover-open");o?.setAttribute("aria-expanded",String(i)),o?.setAttribute("daa-ll",i?"header|Close":"header|Open")})})},Qe=n=>{P.addEventListener("change",()=>{V(n)})},na=n=>{[...n.querySelector(".feds-utilities #universal-nav")?.children??[]].forEach(e=>{e.addEventListener("click",()=>V(n)),e.addEventListener("keydown",a=>{a.key==="Enter"&&V(n)})})};var ea=()=>{},aa=n=>{let e=n.closest("header");if(!e)return;let a=n.querySelector("#feds-menu-wrapper"),r=()=>a?.matches(":popover-open")??!1,t=()=>window.scrollY>100,o=()=>{if(r()){e.classList.remove("feds-header-scrolled");return}if(t()){e.classList.add("feds-header-scrolled");return}e.classList.remove("feds-header-scrolled")};o(),window.addEventListener("scroll",o,{passive:!0}),a?.addEventListener("toggle",o)},ra=(n,e)=>{let a=n.closest("header");a!==null&&a.setAttribute("daa-lh",`gnav|${In()}${e}`)};export{bo as main,Ye as postRenderingTasks,Ve as renderGnav,Ke as renderGnavString};
