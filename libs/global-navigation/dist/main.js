...
`};var q={elementNull:"Error when parsing Brand. Element is null",noLinks:"Error when parsing Brand. No links found",noPrimaryLink:"Error when parsing Brand. No primary link found"},U=/(\.png|\.jpg|\.jpeg|\.svg)/i,Ee=n=>{let e=n.querySelector("picture img")?.getAttribute("src")??null;if(e!==null&&e!=="")return e;let t=n.textContent?.trim();if(t!==void 0&&t!==""&&U.test(t)){let o=t.split("|")[0]?.trim();if(o!==void 0&&o!=="")return o}let r=n.getAttribute("href");return r!==null&&r!==""&&U.test(r)?r:null},Me=n=>{let e=n.textContent?.trim();if(e?.includes("|")===!0){let r=e.split("|")[1]?.trim();if(r)return r}return n.querySelector("img")?.getAttribute("alt")??""},ln=n=>{if(n===null)throw new l(q.elementNull);let e=n.querySelector(".gnav-brand");if(e===null)throw new l(q.elementNull);let t=[...e.querySelectorAll("a")];if(t.length===0)throw new l(q.noLinks);let r=t.find(u=>{let m=u.textContent??"";return!U.test(u.href)&&!U.test(m)});if(!r)throw new l(q.noPrimaryLink);let o=e.matches(".brand-image-only"),a=e.matches(".no-logo"),i=e.matches(".image-only"),s=!a,c=!o&&!i,d=t.filter(u=>{let m=u.textContent??"";return U.test(u.href)||U.test(m)}),[f,g,k]=(()=>{let u=o?H.brand:H.company,[m=null,A=null]=[...e.querySelectorAll('picture img[src$=".svg"]')].map(Q=>Q?.src).filter(Q=>Q?.length>0),[O=null,ce=null]=d.map(Ee),de=d[0]instanceof Element?Me(d[0]):r.textContent?.trim()??"";return[O??m??u,ce??A,de]})(),b=r.textContent?.trim()??"",x=r.href;if(!s&&!c)return[{type:"Brand",data:{type:"NoRender"}},[]];let w=(u,m)=>{let A=m!=null&&m!=="";return En()&&A?m:u},P=f.startsWith("<svg")?{type:"inline-svg",svgContent:w(f,g),alt:k}:{type:"image",src:w(f,g),alt:k};return s&&c?[{type:"Brand",data:{type:"LabelledBrand",href:x,label:b,image:P}},[]]:s&&o?[{type:"Brand",data:{type:"BrandImageOnly",href:x,image:P,alt:k}},[]]:s&&i?[{type:"Brand",data:{type:"ImageOnlyBrand",href:x,image:P,alt:k}},[]]:[{type:"Brand",data:{type:"BrandLabelOnly",href:x,label:b}},[]]};var _e=`.feds-brand-container {
    display: flex;
    flex-shrink: 0;
}
...
`,Wn=document.createElement("style");Wn.textContent=qe;document.head.appendChild(Wn);var Zn=({card:n})=>We(n),We=({bgImageAlt:n,bgImageSrc:e,iconAlt:t,iconSrc:r,title:o,cta:a})=>`
...
`,Qn=(n,e,t)=>{let r=`
  ...
  `.trim(),o="";switch(n.type){case"ProductList":o=Yn(n);break;case"GnavCards":o=Kn(n);break;default:}return`${r}${o}`};var ne={elementNull:"Error when parsing text. Element is null",textContentNull:"Error when parsing text. Element has no textContent"},ee=n=>{if(n===null)return[{type:"Text",content:""},[new p(ne.elementNull,"Minor")]];let e=n.textContent;return e===null?[{type:"Text",content:""},[new p(ne.textContentNull,"Minor")]]:[{type:"Text",content:e},[]]};var te=({content:n})=>n;var re=n=>{if(n===null)throw new l(Ye.elementNull);if(n.querySelector(".gnav-brand")!==null)return ln(n);let t=n.querySelector(".large-menu");return t!==null?zn(t):n.querySelector("strong")!==null?un(n):n.querySelector("em")!==null?_(n):n.querySelector("a")===null?ee(n):h(n.querySelector("a"))},gn=n=>{switch(n.type){case"Text":return te(n);case"Link":return S(n);case"SecondaryCTA":return T(n);case"PrimaryCTA":return fn(n);case"Brand":return dn(n);case"MegaMenu":return Xn(n);default:return console.error(`Failed to recognize component: ${n}`),""}},Ye={elementNull:"Element is null"};var oe=(n,e)=>{let[t,r]=C([...document.querySelectorAll(".breadcrumbs ul > li > a")??[]],h),[o,a]=C([...n.children],re),i=n.querySelector(".product-entry-cta"),[s,c]=(()=>{try{return In(i)}catch{return[null,[]]}})(),d=!1,f=[r,a,c].flat();return{breadcrumbs:t,components:o,productCTA:s,localnav:d,errors:f,unavEnabled:e}};var Xe=n=>{let e=[...n.querySelectorAll('.tabs button[role="tab"]')],t=[...n.querySelectorAll(".tab-content ul")],r=e.map((o,a)=>()=>{e.forEach(i=>{i.setAttribute("aria-selected","false")}),t.forEach(i=>{i.setAttribute("hidden","true")}),t[a]?.removeAttribute("hidden"),o.setAttribute("aria-selected","true")});return e.forEach((o,a)=>{o.addEventListener("click",r[a])}),()=>{e.forEach((o,a)=>{o.removeEventListener("click",r[a])})}};var ie=Xe;var Qe=n=>()=>console.log(n),nt=n=>()=>console.log(n),ae=xn({mobileEventListeners:nt,desktopEventListeners:Qe});var se=async({gnavSource:n,asideSource:e})=>{let t=await R(n);if(t instanceof l)return t;let r=await R(e);return{mainNav:t,aside:r}};var et=`/**
...
`,le=document.createElement("style");le.textContent=et;document.head.appendChild(le);var bi=async n=>{let{gnavSource:e,mountpoint:t,unavEnabled:r,miloConfig:o,personalization:a}=n;if(!(e instanceof URL))throw v(`gnavSource is invalid: ${e}`),new l("gnavSource needs to be a URL object");try{on(o)}catch(f){throw v(`Failed to initialize MiloConfig: ${f}`),new l(`Failed to initialize MiloConfig: ${f}`)}wn(a),vn(mn(n));let i=await se(n);if(i instanceof l)throw v(i.message),i;let{mainNav:s,aside:c}=i;if(s instanceof l)throw v(s.message),s;let d=oe(s,r);if(d instanceof l)throw v(d.message),d;return await tt(d)(t),ot(n)},tt=n=>async e=>{let t=rt(n);e.innerHTML=t,e.classList.add("site-pivot");let r=[...e.querySelectorAll(".mega-menu ~ .feds-popup")];r.forEach(s=>{s.innerHTML=""});let o=n.components.filter(s=>s.type==="MegaMenu"),a=o.map(s=>s.content),i=await Promise.all(a.map(async(s,c)=>{let[d,f]=await s,g=o[c].title;return r[c].innerHTML=Qn(d,r[c].id,g),f}).flat());return e},rt=({components:n,productCTA:e,unavEnabled:t})=>`
<nav>
  <ul>
