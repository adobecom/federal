...
`,nn=document.createElement("style");nn.textContent=Fn;document.head.appendChild(nn);var Jo=async e=>{let{gnavSource:n,mountpoint:t,unavEnabled:r,miloConfig:o,personalization:i}=e;if(!(n instanceof URL))throw v(`gnavSource is invalid: ${n}`),new l("gnavSource needs to be a URL object");try{te(o)}catch(f){throw v(`Failed to initialize MiloConfig: ${f}`),new l(`Failed to initialize MiloConfig: ${f}`)}be(i),ge(ue(e));let a=await en(e);if(a instanceof l)throw v(a.message),a;let{mainNav:s,aside:c}=a;if(s instanceof l)throw v(s.message),s;let d=Ye(s,r);if(d instanceof l)throw v(d.message),d;return await qn(d)(t),Zn(e)},qn=e=>async n=>{let t=Wn(e);n.innerHTML=t,n.classList.add("site-pivot");let r=[...n.querySelectorAll(".mega-menu ~ .feds-popup")];r.forEach(s=>{s.innerHTML=""});let o=e.components.filter(s=>s.type==="MegaMenu"),i=o.map(s=>s.content),a=await Promise.all(i.map(async(s,c)=>{let[d,f]=await s,g=o[c].title;return r[c].innerHTML=We(d,r[c].id,g),f}).flat());return n},Wn=({components:e,productCTA:n,unavEnabled:t})=>`
<nav>
  <ul>
