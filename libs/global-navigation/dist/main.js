...
`,jn=n=>n.type==="PrimaryCTA"?xn(n):_(n);var D=({text:n,href:e,daaLl:r})=>`<a class="feds-link" href="${e}"${C(null,r??n)}>${n}</a>`;var Gn=n=>{let[e,r]=Be(n);return[{type:"LinksCard",card:e},r]},Be=n=>{let e=n.querySelector("h2, h3, h4")||null;if(!e)throw new d("Expected links card title");let r=n.querySelector("em > a"),t=[...n.querySelectorAll("a")].filter(l=>l!==r);if(t.length===0)throw new d("Expected at least one link");let[a,o]=M(t,E),[i,s]=(()=>{try{return A(n)}catch{return[null,[]]}})();return i&&(i.daaLl=`${e.textContent??""} - ${i?.daaLl}`),[{type:"LinksCardItem",title:e.textContent??"",links:a,footerCTA:i},[...o,...s]]};var qn=n=>{let e=[...n.querySelectorAll("li > div")],r=[...n.querySelectorAll("li > a")],[t,a]=M(e,Oe),[o,i]=M(r,E);return[{type:"ProductList",categories:t,links:o},[...a,...i]]},Oe=n=>{let e=n.firstElementChild;if(e?.nodeName!=="H2")throw new d("Expected H2");let r=e.textContent??"",t=e.textContent??"",a=Pn(e),[o,i]=M(a,pn);return[{type:"ProductCategory",name:r,daaLl:t,links:o},i]};var Fn=n=>{let[e,r]=je(n);return[{type:"FeaturedCard",card:e},r]},je=n=>{let e=n.querySelector("h5")||null;if(!e)throw new f("Eye brow element not found");let r=n.querySelector("h4")||null,t=r?.nextElementSibling||null;if(!r)throw new d("Expected title");if(!t)throw new d("Expected subtitle");let a=t.nextElementSibling?.firstElementChild||null;if(!a)throw new d("Expected card link after subtitle");let[o,i]=E(a),[s,l]=A(n);return[{type:"Card",title:r.textContent??"",subtitle:t.textContent??"",bodyLink:o,footerCTA:s,eyeBrow:e.textContent??""},[...l,...i]]};var T={MissingBackgroundImageSection:"Promo card is missing background image section",MissingBackgroundImage:"Promo card is missing background image",MissingBackgroundImageAlt:"Promo card background image is missing alt text",MissingBackgroundImageSrc:"Promo card background image is missing src",MissingContentSection:"Promo card is missing card details section",MissingIcon:"Promo card is missing icon",MissingIconSrc:"Promo card icon is missing src",MissingIconAlt:"Promo card icon is missing alt text",MissingTitleElement:"Promo card is missing title element",MissingTitleText:"Promo card is missing title text",MissingSecondaryCtaAnchor:"Promo card is missing secondary CTA anchor",MissingPriceLink:"Promo card is missing price link"},Wn=n=>{let[e,r]=n.querySelectorAll(":scope > div"),t=new Set;if(e===void 0)throw new d(T.MissingBackgroundImageSection);let a=e.querySelector(":scope picture:not(:scope p picture) img")??null;a===null&&t.add(new f(T.MissingBackgroundImage));let o=a?.getAttribute("alt")??"";o===""&&t.add(new f(T.MissingBackgroundImageAlt));let i=a?.getAttribute("src")??"";if(i===""&&t.add(new f(T.MissingBackgroundImageSrc)),r===void 0)throw new d(T.MissingContentSection);let s=r.querySelector('a[href$=".svg"]')??null;s===null&&t.add(new f(T.MissingIcon));let[l,c]=(s?.textContent?.split("|")??["",""]).map(w=>w.trim());l===""&&t.add(new f(T.MissingIconSrc)),c===""&&t.add(new f(T.MissingIconAlt));let b=r.querySelector('p > a:not([href$=".svg"])')??null,x=b?.textContent?.trim()??"",g=b?.getAttribute("href")??"",p=g?Mn(g):!1;b===null&&t.add(new f(T.MissingPriceLink));let u=r.querySelector("p > strong")??null;if(u===null)throw new d(T.MissingTitleElement);let v=u?.textContent??"";v===""&&t.add(new f(T.MissingTitleText)),r.querySelector("em > a")===null&&t.add(new f(T.MissingSecondaryCtaAnchor));let[m,h]=(()=>{try{return A(n)}catch{return[null,[]]}})();return h.forEach(w=>t.add(w)),m&&(m.daaLl=`${v} - ${m?.daaLl}`),[{type:"PromoCard",card:{bgImageAlt:o,bgImageSrc:i,iconAlt:c,iconSrc:l,title:v,cta:m,priceText:x,priceHref:g,isPriceMerchLink:p}},[...t]]};var Kn=n=>{let e=new Set;if(n===null)throw new d(Zn.elementNull);let r=n.querySelector("h2")?.textContent??"";r===""&&e.add(new f(Zn.noTitle));let t=(async()=>{try{let a=n.querySelector("a"),o=new URL(a?.href??""),i=await H(o);if(i instanceof d)throw new Error(i.message);let s=await An(i);if(s instanceof d)throw new Error(s.message);return Sn(o.href,s),n.classList.contains("product-list")?qn(s):Ge(s)}catch(a){throw new d(a?.message)}})();if(t instanceof d)throw t;return[{type:"MegaMenu",title:r,content:t},[...e]]},Zn={elementNull:"Element is null",noTitle:"Large Menu has no Title"},Ge=n=>{let e=[...n.children];if(e.length===0)throw new d("No mega menu items found (did you forget to add them correctly?)");let[r,t]=M(e,a=>qe(a));if(r.length===0)throw new d("Failed to parse gnav cards sections");return[{type:"GnavCards",sections:r},t]},qe=n=>{let e=[...n.querySelectorAll(".featured-card, .links-card, .promo-card")];if(e.length===0)throw new d("Column contains no cards (did you forget to label them correctly?)");let[r,t]=M(e,a=>Fe(a));if(r.length===0)throw new d("Failed to parse cards in column");return[{type:"GnavColumn",cards:r},t]},Fe=n=>{if(n.classList.contains("featured-card"))return Fn(n);if(n.classList.contains("links-card"))return Gn(n);if(n.classList.contains("promo-card"))return Wn(n);throw new d("Unsupported gnav cards section")};var Vn=({card:n})=>We(n),We=({title:n,subtitle:e,eyeBrow:r,footerCTA:t,bodyLink:a})=>`
...
`.trim();var Xn=({card:n})=>Ze(n),Ze=({title:n,links:e,footerCTA:r})=>`
...
`.trim();var Yn=({card:n})=>Ke(n),Ke=({bgImageAlt:n,bgImageSrc:e,iconAlt:r,iconSrc:t,title:a,cta:o,priceText:i,priceHref:s,isPriceMerchLink:l})=>`
...
`.trim();var Ve=n=>{switch(n.type){case"FeaturedCard":return Vn(n);case"LinksCard":return Xn(n);case"PromoCard":return Yn(n);default:}return""},Jn=({sections:n})=>`
...
`;var Qn=({categories:n,links:e})=>{let r=`
  ...
  <div id="${j(n)}" popover class="feds-popup">
  </div>
`,ee=(n,e,r)=>{let t=`
        <button
          type="button"
...
`,ce=document.createElement("style");ce.textContent=Je;document.head.appendChild(ce);var bo=async n=>{let{gnavSource:e,mountpoint:r,unavEnabled:t,miloConfig:a,personalization:o}=n;if(!(e instanceof URL))throw L(`gnavSource is invalid: ${e}`),new d("gnavSource needs to be a URL object");try{cn(a)}catch(b){throw L(`Failed to initialize MiloConfig: ${b}`),new d(`Failed to initialize MiloConfig: ${b}`)}Tn(o),Ln(Cn(n));let i=await de(n);if(i instanceof d)throw L(i.message),i;let{mainNav:s,aside:l}=i;if(s instanceof d)throw L(s.message),s;let c=ie(s,t);if(c instanceof d)throw L(c.message),c;return await Qe(c)(r),er(n)},Qe=n=>async e=>{let r=nr(n);e.innerHTML=r,e.classList.add("site-pivot");let t=[...e.querySelectorAll(".mega-menu ~ .feds-popup")];t.forEach(s=>{s.innerHTML=""});let a=n.components.filter(s=>s.type==="MegaMenu"),o=a.map(s=>s.content),i=await Promise.all(o.map(async(s,l)=>{let[c,b]=await s,x=a[l].title;return t[l].innerHTML=ee(c,t[l].id,x),b}).flat());return e},nr=({components:n,productCTA:e,unavEnabled:r})=>`
<nav>
  <ul>
...
`,er=async n=>{let e=new Set,r=await tn(n.mountpoint);r instanceof f?(e.add(r),L(r.message)):r.errors.forEach(o=>e.add(o)),se(n.mountpoint),bn(n.mountpoint),rr(n.mountpoint),tr(n.mountpoint),ar(n.mountpoint),ir(n.mountpoint),sr(n.mountpoint,n.mepMartech??""),(await le(n.mountpoint)).forEach(o=>{e.add(o),L(o.message)});let a=r instanceof f?()=>{}:r.reloadUnav;return{closeEverything:or,reloadUnav:a,errors:e,setGnavTopPosition:o=>{},getGnavTopPosition:()=>0}},rr=n=>{let e=n.querySelector("#feds-menu-wrapper"),r=n.querySelector(".feds-nav-toggle");e?.addEventListener("toggle",()=>{let a=e.matches(":popover-open");r?.setAttribute("aria-expanded",String(a)),r?.setAttribute("daa-ll",a?"hamburgermenu|close":"hamburgermenu|open"),e.setAttribute("aria-hidden",String(!a)),a&&e.classList.add("feds-menu-active")}),e?.addEventListener("transitionend",()=>{e.matches(":popover-open")||e.classList.remove("feds-menu-active")}),n.querySelectorAll(".feds-popup[popover]").forEach(a=>{a.addEventListener("toggle",()=>{let o=n.querySelector(`[popovertarget="${a.id}"]`),i=a.matches(":popover-open");o?.setAttribute("aria-expanded",String(i)),o?.setAttribute("daa-ll",i?"header|Close":"header|Open")})})},tr=n=>{B.addEventListener("change",()=>{K(n)})},ar=n=>{[...n.querySelector(".feds-utilities #universal-nav")?.children??[]].forEach(e=>{e.addEventListener("click",()=>K(n)),e.addEventListener("keydown",r=>{r.key==="Enter"&&K(n)})})},or=()=>{},ir=n=>{let e=n.closest("header");if(!e)return;let r=n.querySelector("#feds-menu-wrapper"),t=()=>r?.matches(":popover-open")??!1,a=()=>window.scrollY>100,o=()=>{if(t()){e.classList.remove("feds-header-scrolled");return}if(a()){e.classList.add("feds-header-scrolled");return}e.classList.remove("feds-header-scrolled")};o(),window.addEventListener("scroll",o,{passive:!0}),r?.addEventListener("toggle",o)},sr=(n,e)=>{let r=n.closest("header");r!==null&&r.setAttribute("daa-lh",`gnav|${Hn()}${e}`)};export{bo as main,er as postRenderingTasks,Qe as renderGnav,nr as renderGnavString};
