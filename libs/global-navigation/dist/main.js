...
`,de=document.createElement("style");de.textContent=Ye;document.head.appendChild(de);var vo=async n=>{let{gnavSource:e,mountpoint:r,unavEnabled:t,miloConfig:a,personalization:o}=n;if(!(e instanceof URL))throw k(`gnavSource is invalid: ${e}`),new l("gnavSource needs to be a URL object");try{cn(a)}catch(y){throw k(`Failed to initialize MiloConfig: ${y}`),new l(`Failed to initialize MiloConfig: ${y}`)}Tn(o),Ln(Cn(n));let i=await le(n);if(i instanceof l)throw k(i.message),i;let{mainNav:s,aside:d}=i;if(s instanceof l)throw k(s.message),s;let c=oe(s,t);if(c instanceof l)throw k(c.message),c;return await Je(c)(r),nr(n)},Je=n=>async e=>{let r=Qe(n);e.innerHTML=r,e.classList.add("site-pivot");let t=[...e.querySelectorAll(".mega-menu ~ .feds-popup")];t.forEach(s=>{s.innerHTML=""});let a=n.components.filter(s=>s.type==="MegaMenu"),o=a.map(s=>s.content),i=await Promise.all(o.map(async(s,d)=>{let[c,y]=await s,w=a[d].title;return t[d].innerHTML=ne(c,t[d].id,w),y}).flat());return e},Qe=({components:n,productCTA:e,unavEnabled:r})=>`
<nav>
  <ul>
